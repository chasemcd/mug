  ...
  artifacts:
    - path: "interactive_gym/server/game_manager.py"
      ...
      removed:
        ...
        - "_add_subject_to_specific_game method"
...
WHY: These methods are only reachable through the group reunion path which is being removed. Keeping them would leave dead code.
  </action>
  <verify>
    - `grep -n "group_waitrooms\|group_wait_start_times" interactive_gym/server/game_manager.py` returns no results
    - `grep -n "_join_or_wait_for_group\|_create_game_for_group\|_add_subject_to_specific_game" interactive_gym/server/game_manager.py` returns no results
  ...
  <done>GameManager has single game creation path through _add_to_fifo_queue; all group reunion code removed; warning logged when wait_for_known_group=True</done>
</task>
...
2. **No dead code**: No methods referencing group waitrooms exist
...
2. All group reunion methods and data structures removed from GameManager
3. app.py and aggregator.py have no references to removed code
4. wait_for_known_group=True logs warning but doesn't crash
5. GAME-01, GAME-02, GAME-03, GAME-04 marked complete in REQUIREMENTS.md
