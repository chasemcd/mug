...
must_haves:
  truths:
    ...
    - "Session is destroyed (del self.games[game_id]) when cleanup_game() is called after ENDED transition"
  artifacts:
  ...
  key_links:
    ...
    - from: "GameManager.cleanup_game"
      ...
      via: "del self.games[game_id]"
      pattern: "del self\\.games\\[game_id\\]"
---

<objective>
Add explicit session lifecycle states to RemoteGameV2 so session state is always queryable and transitions are validated.

Purpose: SESS-01 requires explicit states (WAITING -> MATCHED -> VALIDATING -> PLAYING -> ENDED). Currently state is tracked implicitly across multiple data structures (`waiting_games`, `active_games`, `is_active`). This creates fragmented state that's hard to query and debug. SESS-02 (session destroyed when game ends) is already implemented by Phase 52's cleanup_game() which does `del self.games[game_id]` - this plan adds ENDED transition before that destruction.
...
<task type="auto">
  ...
  <action>
    ...
    ```
    Note: This ENDED transition happens BEFORE `del self.games[game_id]` which destroys the session object (SESS-02).
  ...
  <done>
    GameManager calls transition_to() at all lifecycle points: WAITING at creation (implicit), MATCHED when ready, PLAYING at game start (non-pyodide), ENDED at cleanup. Session destruction (`del self.games[game_id]`) follows ENDED transition per SESS-02.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    For PyodideGameCoordinator, the coordinator doesn't hold RemoteGameV2 references directly - it has its own PyodideGameState. The transition needs to happen via a callback by getting the game from GameManager.

    **Approach: Add game_manager_getter to coordinator**
    ...
    **In app.py:** Update PyodideGameCoordinator instantiation to pass the game_manager getter:
    ...
    def get_game_manager_for_game(game_id):
        ...
        # Note: stagers is module-level dict defined earlier in app.py
  ...
  <verify>
    ...
    2. Verify state transitions are logged during test runs:
    ```bash
    cd /Users/chasemcd/Repositories/interactive-gym && python -m pytest tests/e2e/test_infrastructure.py::test_multi_player_game_start -v --timeout=60 2>&1 | grep -E "Session.*->|WAITING|MATCHED|VALIDATING|PLAYING|ENDED"
    ```
    Expected: Log lines showing state transitions (e.g., "Session game_xxx: WAITING -> MATCHED -> VALIDATING -> PLAYING")
  </verify>
  <done>
    P2P validation phase transitions RemoteGameV2 to VALIDATING when start_validation() called, and to PLAYING when validation completes. State transitions are logged and visible in test output. All session state transitions are explicit and logged.
  </done>
</task>
...
4. SESS-02 verification (session destroyed when game ends):
   ```bash
   grep -n "del self.games\[game_id\]" interactive_gym/server/game_manager.py
   ```
   Expected: Line showing session deletion in cleanup_game() AFTER transition_to(ENDED)
</verification>
...
- [ ] PyodideGameCoordinator triggers VALIDATING and PLAYING transitions for P2P games via game_manager_getter
- [ ] E2E tests pass (no regressions)
- [ ] State transitions are logged and visible in test output
- [ ] SESS-01 requirement met: explicit states queryable via game.session_state
- [ ] SESS-02 requirement met: session destroyed (`del self.games[game_id]`) when cleanup_game() called after ENDED transition (implemented by Phase 52, verified by this plan)
</success_criteria>
