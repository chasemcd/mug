...
This example complements the :doc:`../quick_start_multiplayer` guide by showing a more complex P2P multiplayer setup with sprite-based rendering, latency-aware matchmaking, and participant management features.
...
.. list-table::
   ...
   * - **Execution Mode**
     - Client-side (P2P with GGPO)
   * - **Players**
     - 2 humans
   * - **Sync**
     - GGPO rollback netcode over WebRTC DataChannel
   * - **Environment**
...
The environment must implement ``get_state()`` and ``set_state()`` for GGPO rollback. See the :doc:`../quick_start_multiplayer` guide for details.
...
For more details on GGPO, see the :doc:`../quick_start_multiplayer` guide.
...
For the server-authoritative version of this example, see :doc:`overcooked_multiplayer`.
...
- **Multiplayer quickstart**: :doc:`../quick_start_multiplayer` for a simpler P2P example with full GGPO explanation
- **Server-authoritative version**: :doc:`overcooked_multiplayer` for when the environment can't run in Pyodide
- **Human-AI version**: :doc:`overcooked_human_ai` for single-player with trained AI partners
- **Server mode details**: :doc:`../core_concepts/server_mode` for deployment and scaling
