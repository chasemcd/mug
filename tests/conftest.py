...
@pytest.fixture(scope="function")
def flask_server_slimevb_human_heuristic():
    """
    Start Flask server for slime volleyball human-heuristic policy testing.
    ...
    process = subprocess.Popen(
        [
            "python",
            "-m",
            "tests.fixtures.slimevb_human_heuristic_test",
            "--port",
            str(port),
        ],
        stdout=subprocess.DEVNULL,
    ...
    for attempt in range(max_retries):
        ...
        if process.poll() is not None:
            stderr = process.stderr.read() if process.stderr else b""
            raise RuntimeError(
                f"SlimeVB human-heuristic server exited unexpectedly (code {process.returncode}).\n"
                f"stderr: {stderr.decode()}"
            )
        ...
        _teardown_server(process, port)
        raise RuntimeError(
            f"SlimeVB human-heuristic server failed to start after {max_retries} retries"
        )
