"""Schema-authority conformance for the tools (API-14) models.

The frozen API-14 bundle is the authority. This test binds the tool models to it
through the golden fixtures: what the schema accepts, the model parses; what the
schema rejects, the model rejects. Two invariants (``toolEvidence``,
``approvalGate``) live in ``ToolResult`` as validators, so at the semantic layer
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-14/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "ToolVersion": ToolVersion.model_validate,
    "ToolCall": ToolCall.model_validate,
    "ToolApproval": ToolApproval.model_validate,
    "ToolResult": ToolResult.model_validate,
    "EnvironmentCommandMailbox": EnvironmentCommandMailbox.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The tool model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no tool validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-14 bundle digest."""
    schema = tools_schema()
    version = _instance("valid/tool-version.minimal-static.json")
    assert version["schema"]["digest"]["hex"] == schema.bundle_digest
