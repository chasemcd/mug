"""Schema-authority conformance for the platform (API-02) models.

The frozen API-02 bundle is the authority. This test binds the platform models to
it through the golden fixtures: what the schema accepts, the model parses; what the
schema rejects, the model rejects. Four invariants (``duplicateSecretKey``,
``providerSecretBinding``, ``duplicateExecutionSlot``, ``satisfiedConsistency`` with
its ``revisionDeploymentMismatch`` sibling) live in the models as validators, so at
the semantic layer the model is stricter than the raw schema and the test compares
to the manifest.
"""
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-02/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "DeploymentRequirement": DeploymentRequirement.model_validate,
    "Deployment": Deployment.model_validate,
    "DeploymentRevision": DeploymentRevision.model_validate,
    "ClientDeploymentProjection": ClientDeploymentProjection.model_validate,
    "SatisfactionReport": SatisfactionReport.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The platform model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no platform validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-02 bundle digest."""
    schema = platform_schema()
    deployment = _instance("valid/deployment.live-static.json")
    assert deployment["schema"]["digest"]["hex"] == schema.bundle_digest
