"""Schema-authority conformance for the memory (API-15) models.

The frozen API-15 bundle is the authority. This test binds the memory models to it
through the golden fixtures: what the schema accepts, the model parses; what the
schema rejects, the model rejects. One invariant (``memoryVersion``) lives in the
models as a validator, so at the semantic layer the model is stricter than the raw
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-15/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "MemoryScope": MemoryScope.model_validate,
    "MemoryRead": MemoryRead.model_validate,
    "MemoryProposal": MemoryProposal.model_validate,
    "MemoryCommit": MemoryCommit.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The memory model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no memory validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-15 bundle digest."""
    schema = memory_schema()
    scope = _instance("valid/memory-scope.minimal-static.json")
    assert scope["schema"]["digest"]["hex"] == schema.bundle_digest
