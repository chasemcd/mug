"""Schema-authority conformance for the preferences (API-18) models.

The frozen API-18 bundle is the authority. This test binds the preference models
to it through the golden fixtures: what the schema accepts, the model parses; what
the schema rejects, the model rejects. Two invariants
(``duplicateDisplayOrder``, ``choiceNotPresented``) live in the models as
validators, so at the semantic layer the model is stricter than the raw schema and
the test compares to the manifest.
"""
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-18/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "PreferenceProtocol": PreferenceProtocol.model_validate,
    "CandidateRef": CandidateRef.model_validate,
    "PreferenceAssignment": PreferenceAssignment.model_validate,
    "PreferenceResponse": PreferenceResponse.model_validate,
    "QualityEvidence": QualityEvidence.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The preference model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no preference validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-18 bundle digest."""
    schema = preferences_schema()
    protocol = _instance("valid/preference-protocol.minimal-static.json")
    assert protocol["schema"]["digest"]["hex"] == schema.bundle_digest
