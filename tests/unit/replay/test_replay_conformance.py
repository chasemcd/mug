"""Schema-authority conformance for the replay (API-16) models.

The frozen API-16 bundle is the authority. This test binds the replay models to
it through the golden fixtures: what the schema accepts, the model parses; what
the schema rejects, the model rejects. Several invariants (for example
``replayCapability``, ``p2pFinalityOrder``, ``stateHashResult``, and
``replayValidity``) live in the models as validators, so at the semantic layer
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-16/fixtures/v0"
)

_STATE_HASH_CHECK: TypeAdapter[StateHashCheck] = TypeAdapter(StateHashCheck)
_FINALITY_OUTCOME: TypeAdapter[P2PFinalityOutcome] = TypeAdapter(P2PFinalityOutcome)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "ReplayManifest": ReplayManifest.model_validate,
    "DecisionTape": DecisionTape.model_validate,
    "ReplayBundleValidation": ReplayBundleValidation.model_validate,
    "DeterminismDeclaration": DeterminismDeclaration.model_validate,
    "ExperiencedFrameLineageEntry": ExperiencedFrameLineageEntry.model_validate,
    "StateHashCheck": _STATE_HASH_CHECK.validate_python,
    "P2PFinalityOutcome": _FINALITY_OUTCOME.validate_python,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The replay model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no replay validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-16 bundle digest."""
    schema = replay_schema()
    manifest = _instance("valid/replay-manifest.minimal-static.json")
    assert manifest["schema"]["digest"]["hex"] == schema.bundle_digest
