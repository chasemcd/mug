"""Schema-authority conformance for the game (API-07) models.

The frozen API-07 bundle is the authority. This test binds the game models to it
through the golden fixtures: what the schema accepts, the model parses; what the
schema rejects, the model rejects. Several cross-field invariants live in the
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-07/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "EnvFactory": EnvFactory.model_validate,
    "ExecutionMode": ExecutionMode.model_validate,
    "GameTransition": GameTransition.model_validate,
    "RenderPacket": RenderPacket.model_validate,
    "EpisodeBoundary": EpisodeBoundary.model_validate,
    "P2PFrameFinality": P2PFrameFinality.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The game model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no game validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-07 bundle digest."""
    schema = game_schema()
    transition = _instance("valid/game-transition.minimal-static.json")
    assert transition["schema"]["digest"]["hex"] == schema.bundle_digest
