"""Schema-authority conformance for the identity (API-03) models.

The frozen API-03 bundle is the authority. This test binds the identity models to
it through the golden fixtures: what the schema accepts, the model parses; what the
schema rejects, the model rejects. Two invariants (``principalKind``,
``piiClassification``) live in the models as validators, so at the semantic layer
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-03/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "Enrollment": Enrollment.model_validate,
    "LaunchTicket": LaunchTicket.model_validate,
    "ExternalIdentityLink": ExternalIdentityLink.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The identity model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no identity validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-03 bundle digest."""
    schema = identity_schema()
    enrollment = _instance("valid/enrollment.minimal-static.json")
    assert enrollment["schema"]["digest"]["hex"] == schema.bundle_digest
