"""Schema-authority conformance for the export (API-19) models.

The frozen API-19 bundle is the authority. This test binds the export models to
it through the golden fixtures: what the schema accepts, the model parses; what
the schema rejects, the model rejects. Three invariants (``git patch``,
``exportLineage``, ``derivedExportSource``) live in the models as validators, so
at the semantic layer the model is stricter than the raw schema and the test
...
_FIXTURES = (
    Path(__file__).resolve().parents[3] / "docs/architecture/phase-0/api-19/fixtures/v0"
)

_VALIDATORS: dict[str, Callable[[Any], Any]] = {
    "ExportRequest": ExportRequest.model_validate,
    "ExportBundle": ExportBundle.model_validate,
    "LineageRecord": LineageRecord.model_validate,
    "DatasetSchemaBinding": DatasetSchemaBinding.model_validate,
}
...
def test_model_matches_manifest(case: dict[str, Any]) -> None:
    """The export model verdict matches the manifest for every case."""
    def_name = _def_name(case["schema_ref"])
    validate = _VALIDATORS.get(def_name)
    assert validate is not None, f"no export validator for {def_name}"
...
def test_pinned_schema_ref_matches_bundle_digest() -> None:
    """A valid fixture's schema reference pins the frozen API-19 bundle digest."""
    schema = export_schema()
    request = _instance("valid/export-request.minimal-static.json")
