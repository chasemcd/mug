"""
Slime Volleyball Human-Heuristic - Test Configuration

Minimal test server for heuristic (Python code) policy E2E testing:
- Single human + heuristic policy partner (ball_chaser)
- Short episode (200 steps ~7s at 30fps)
- No tutorial, no feedback
- Relaxed browser requirements for test automation

Usage:
    python -m tests.fixtures.slimevb_human_heuristic_test

This is used by pytest E2E tests via the conftest.py
flask_server_slimevb_human_heuristic fixture.
"""
...
start_scene = (
    ...
    .display(
        ...
        scene_body="Slime Volleyball heuristic policy test.",
    )
)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Slime Volleyball human-heuristic experiment (TEST CONFIG)"
    )
    parser.add_argument(
        "--port", type=int, default=5714, help="Port number to listen on"
    )
    args = parser.parse_args()

    experiment_config = (
        experiment_config.ExperimentConfig()
        .experiment(stager=stager, experiment_id="slimevb_human_heuristic_test")
